const PERSISTENCE_NAMESPACE = 'carrot-alpha-sdk'
...
export abstract class Cacher {
  ...
  public static get<T extends object | string>(key: string): T | null {
  ...
  public static getOrDefault<T extends object | string>(key: string, defaultValue: T): T {
    const cachedItem = Cacher.get<T>(key)
    return cachedItem || defaultValue
  }

  public static set<T extends object | string>(key: string, value: T, validUntil: number) {
